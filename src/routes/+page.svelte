...
<main class="min-h-screen">
  <header class="bg-gray-900 text-white border-b border-gray-700">
    <div class="max-w-7xl mx-auto px-4 flex justify-between items-center py-4">
      <h1 class="text-xl font-semibold">Open Scholarships for Vietnamese Students</h1>
      <a 
  ...
  <div class="my-8">
    <div class="max-w-7xl mx-auto my-2 px-2">
        <SearchBar 
        bind:searchQuery 
    ...
    <div class="max-w-7xl mx-auto grid grid-cols-[300px_1fr] min-h-[calc(100vh-200px)]">
        <aside class="p-2" aria-label="Filter options">
        <FilterSidebar 
            bind:activeFilters 
        ...
        <section class="px-2" aria-label="Scholarship listings">
        {#if isLoading}
        ...
        {:else if filteredScholarships.length === 0}
            ...
            {#if searchQuery || activeFilters.length > 0}
                <button 
                type="button" 
                class="px-4 py-2 text-sm border border-red-500 text-red-500 hover:bg-red-500 hover:text-white rounded transition-colors"
        ...
        {:else}
            ...
            {#if totalPages > 1}
            <nav class="flex justify-center items-center gap-4 mt-8" aria-label="Pagination">
                <button 
                type="button" 
                class="px-6 py-3 bg-blue-500 text-white font-semibold rounded disabled:bg-gray-300 disabled:cursor-not-allowed hover:bg-blue-600 transition-colors"
